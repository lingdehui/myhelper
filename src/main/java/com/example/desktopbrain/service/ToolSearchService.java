...
public class ToolSearchService {
    ...
    @Tool(description = "一次性列出系统中所有可用工具的完整信息（名称、来源、参数列表）。当 AI 需要了解整体能力时调用此方法，避免多次 searchTool 查询。返回结果包含每个工具的参数类型和约束。")
    public String listAllTools() {
        List<ToolModel> all = toolRegistry.findAllActive();
        if (all.isEmpty()) return "暂无可用工具（工具注册表为空，请检查系统启动日志）";

        StringBuilder sb = new StringBuilder();
        sb.append("共 ").append(all.size()).append(" 个工具:\n\n");

        // 按类型分组显示
        List<ToolModel> javaTools = all.stream().filter(m -> "JAVA".equals(m.type())).toList();
        List<ToolModel> mcpTools = all.stream().filter(m -> "MCP".equals(m.type())).toList();
        List<ToolModel> genTools = all.stream().filter(m -> "GENERATED".equals(m.type())).toList();

        appendGroup(sb, "Java 本地工具", javaTools);
        appendGroup(sb, "MCP 远程工具", mcpTools);
        appendGroup(sb, "自生成工具", genTools);

        log.info("📋 listAllTools → {} 个工具 (Java:{}, MCP:{}, Gen:{})",
                all.size(), javaTools.size(), mcpTools.size(), genTools.size());
        return sb.toString();
    }

    private void appendGroup(StringBuilder sb, String label, List<ToolModel> tools) {
        if (tools.isEmpty()) return;
        sb.append("## ").append(label).append(" (").append(tools.size()).append(" 个)\n");
        for (ToolModel m : tools) {
            sb.append(m.toPromptText()).append("\n\n");
        }
    }
}
