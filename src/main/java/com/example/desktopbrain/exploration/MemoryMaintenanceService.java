 ...
 * </ol>
 *
 * <p>每天凌晨 4:00 执行，cron 可通过 desktopbrain.memory-maintenance.cron 配置。</p>
 *
 * <h3>白名单机制</h3>
...
public class MemoryMaintenanceService {
    ...
    @Scheduled(cron = "${desktopbrain.memory-maintenance.cron:0 0 4 * * ?}")
    ...
    /** 执行清理（价值评分版） */
    public void cleanup() {
        DesktopBrainProperties.MemoryMaintenance mm = props.memoryMaintenance();
    ...
    double computeValueScore(Episode ep, int ageDays, int retentionDays) {
        DesktopBrainProperties.MemoryMaintenance.Weights w = props.memoryMaintenance().weights();
    ...
    private int getRetentionDays(String sourceType) {
        DesktopBrainProperties.MemoryMaintenance.RetentionRules rules =
