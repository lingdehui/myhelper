...
public class SpeechEventLoop {
    ...
    /** 唤醒后尚未收到任何后续命令；用于可解释地记录“唤醒后超时”。 */
    private volatile boolean wakeAwaitingCommand = false;
            ...
            try {
                ...
                if (state == DialogStateMachine.State.IDLE) {
                    ...
                    if (fuzzyMatchWakeWord(text, props.voice().wakeWord())) {
                        runtimeMetrics.recordWakeTrigger();
                        ...
                        if (!turnProcessor.isActive() && dialogStateMachine.getState() == DialogStateMachine.State.LISTENING) {
                            ...
                            if (sc >= timeoutThreshold) {
                                if (wakeAwaitingCommand) {
                                    runtimeMetrics.recordWakeTimeout();
                ...
                followUpHintGiven = false;
                if (wakeAwaitingCommand) {
                    runtimeMetrics.recordWakeConfirmed();
                    wakeAwaitingCommand = false;
                }
    ...
    private boolean fuzzyMatchWakeWord(String text, String wakeWord) {
        ...
        // 默认由配置给出 2；元优化器仅在明确的误触/漏唤醒反馈驱动下，在 0~3 内小步调整。
        int maxDist = Math.min(wLen, runtimeTuning.wakeWordMaxEditDistance());
