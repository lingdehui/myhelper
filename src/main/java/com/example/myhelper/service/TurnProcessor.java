...
import com.example.myhelper.novel.NovelWritingPipeline;
 ...
 *   <li>缓存命中 → ContextUnit 条件可信则直接执行 Unit 树，否则 AI 带参考计划执行</li>
...
public class TurnProcessor {
    ...
    private final NovelWritingPipeline novelWritingPipeline;
    ...
    public TurnProcessor(ToolPlanner toolPlanner,
                          ...
                          UniversalUnitExecutor universalUnitExecutor,
                            ...
                            NovelWritingPipeline novelWritingPipeline,
        ...
        this.novelWritingPipeline = novelWritingPipeline;
    ...
    public void process(ModelRouter modelRouter, ToolCallback[] baseTools,
        ...
        if (batchTarget > 0 && batchNovel != null) {
            runNovelBatch(batchNovel, batchTarget, ttsService);
            return;
        }
    ...
    private String askExploreDecision(ModelRouter modelRouter, String userInput, Unit unit) {
        String prompt = String.format("""
                ...
                String.join(", ", planToolNames(unit)),
    ...
    private void handleCacheHit(ModelRouter modelRouter, ToolCallback[] tools,
        ...
        if (unit.allowsDirectExecution()) {
            handleDirectUnitExecution(modelRouter, tools, effectiveInput, userInput, plan, unit,
    ...
    /** ContextUnit 条件满足时直接执行唯一的 Unit 树。 */
    private void handleDirectUnitExecution(ModelRouter modelRouter, ToolCallback[] tools,
                                            String effectiveInput, String userInput,
                                            ToolPlanner.PlanResult plan, Unit unit,
                                            List<ToolCallLog> toolCallLogs,
                                            long myTurnId, TtsService ttsService) {
        log.info("🚀 Unit 已验证且环境条件可检查，跳过 AI 直接执行 Unit 树");
        UniversalUnitExecutor.ExecutionResult execResult = universalUnitExecutor.executeDirectPlanStep(unit.unitId(), "{}");

        if (execResult.success()) {
            toolCallLogs.addAll(execResult.executedSteps());
            if (currentAiTurnId == myTurnId) {
                toolPlanner.onCacheHitSuccess(userInput, plan);
                String response = "已直接完成（" + execResult.executedSteps().size() + " 步）";
                log.info("🤖 {}", response);
            ...
            toolCallLogs.addAll(execResult.executedSteps());
            String reason = execResult.errorMessage();
            log.info("↩️ Unit 不能直接执行，回到规划器: {}", reason);
            toolPlanner.onCacheHitFailure(userInput, plan, reason, false);
            if (canRetry()) {
                handleCacheFailure(modelRouter, tools, effectiveInput, userInput, plan,
                        toolCallLogs, myTurnId, ttsService, reason);
            }
        }
    }

    private List<String> planToolNames(Unit unit) {
        return unit == null ? List.of() : Optional.ofNullable(toolPlanner.toolNamesOf(unit.unitId())).orElse(List.of());
    }
    ...
    private ToolCallback[] buildLoggedTools(Map<String, ToolCallback> toolMap, List<ToolCallLog> toolCallLogs) {
        return toolMap.values().stream()
                .map(tc -> new LoggingToolCallback(tc, toolCallLogs, generatedToolRegistry, runtimeMetrics))
                .toArray(ToolCallback[]::new);
    }
    ...
    /** 未显式指定章数时只写一个短检查点，避免“一口气”误生成大量未经人工浏览的章节。 */
    private static final int DEFAULT_BATCH_CHAPTERS = 3;
    /** 批量续写硬上限，防止误触发写太多 */
    ...
    /**
     * 自动连续写章节：每章固定走“计划 → 草稿 → 质量门禁 → 最多两次定向修订 → 原子提交”。
     * 质量失败即停止，绝不把未通过的正文当作下一章上下文。
     */
    private void runNovelBatch(String novelName, int target, TtsService ttsService) {
        long myTurnId = aiTurnId.incrementAndGet();
        currentAiTurnId = myTurnId;
        silenceCount = 0;
        ...
        if (current >= target) {
            ...
            while (current < target && currentAiTurnId == myTurnId && safety-- > 0) {
                ...
                try {
                    NovelWritingPipeline.WritingResult result = novelWritingPipeline.writeAndCommit(novelName, next);
                    if (!result.published()) {
                        stopReason = result.message();
                        log.warn("⚠️ 自动续写第 {} 未通过质量门禁，停止: {}", next, stopReason);
                        break;
                    }
                    ...
                    stopReason = "自动写作异常：" + e.getMessage();
        ...
        if (currentAiTurnId == myTurnId) {
            String message = current >= target
                    ? "已自动续写完成，当前共 " + current + " 章"
                    : "自动续写已在第 " + (current + 1) + " 章停止，原因：" + stopReason;
            speakIfPossible(ttsService, message);
        }
        if (currentAiTurnId == myTurnId) {
            currentAiTurnId = -1;
            silenceCount = 0;
