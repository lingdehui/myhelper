...
import com.example.myhelper.optimization.RuntimeMetricsService;
import com.example.myhelper.optimization.RuntimeTuningService;
import com.example.myhelper.registry.ToolModel;
import com.example.myhelper.registry.ToolRegistry;
import com.example.myhelper.world.WorldContextAssembler;
 ...
 *   [Layer 3] 索引路由（分类向量 + 工具向量 → 有界候选集 → 一次 AI 决策）
 *             仅在候选明确且未超出预算时使用；目录仍由分类树完整维护
 *   [Fallback] 多轮目录浏览（原 L1 → L2 → 最终工具选择），路由不足时保留能力边界
 ...
 * <h3>反馈学习</h3>
 * <ul>
 *   <li>命中缓存+成功 → 内存重置失败计数 + Unit 验证计数，达标后允许条件直达</li>
...
public class ToolPlanner {
    ...
    private final RuntimeMetricsService runtimeMetrics;
    private final RuntimeTuningService runtimeTuning;
    ...
    private final WorldContextAssembler worldContextAssembler;
    ...
    public ToolPlanner(ModelRouter modelRouter,
                        ...
                        RuleInductionService ruleInductionService,
                         ...
                         RuntimeTuningService runtimeTuning,
                         WorldContextAssembler worldContextAssembler) {
        ...
        this.worldContextAssembler = worldContextAssembler;
        this.cache = Collections.synchronizedMap(new LinkedHashMap<>(200, 0.75f, true) {
            @Override
            protected boolean removeEldestEntry(Map.Entry<String, CacheEntry> eldest) {
                boolean evict = size() > ToolPlanner.this.runtimeTuning.toolPlannerCacheSize();
                if (evict) ToolPlanner.this.runtimeMetrics.recordCacheEviction();
                return evict;
            }
        });
    }
     ...
     *       先尝试无模型目录路由；候选不足或过宽时，再走完整的树形分类→工具选择流程</li>
     * </ol>
     *
    ...
    public PlanResult plan(String userInput, ToolCallback[] allTools) {
        ...
        try {
            result = planInternal(userInput, allTools);
            return result;
        } catch (RuntimeException e) {
            failed = true;
            throw e;
        } finally {
            runtimeMetrics.recordPlanning(result != null && result.fromCache(),
                    System.currentTimeMillis() - startedAt, failed);
        }
    }

    /** 原三层规划实现；由外层统一采集耗时、缓存命中和失败指标。 */
    private PlanResult planInternal(String userInput, ToolCallback[] allTools) {
        ...
        if (matchedUnit.isPresent()) {
            ...
            List<String> toolNames = unitStore.getToolNamesOf(unit.unitId());
            // 回填内存缓存（带 Unit 引用），下次相同输入直接内存命中
        ...
        // 注意 unitId=null、unit=null：下次命中走 handleNewPlan 正常 AI 执行。
    ...
    /**
     * 缓存命中且成功：重置内存失败计数 + Unit 成功计数，达标后允许条件直达。
     *
     * <p>Unit 是 immutable record，直达状态升级由 {@link UnitStore#incrementSuccess} 内部
     ...
     * </ul>
     *
     * <p>优先使用“分类/工具索引召回 + 一次最终决策”的快速路径。只有在分类命中可信、
     * 候选数量有界时才启用；否则完整保留以下多轮目录浏览逻辑，以防索引漏召回导致
     * 能力退化。</p>
     *
     * <h3>目录浏览回退的异常处理</h3>
     * <ul>
    ...
    private PlanResult doPlan(String userInput, ToolCallback[] allTools, String failureReason,
        ...
        String contextPrefix = envService.getOsInfo() + worldContextAssembler.forPlanning();

        // === 快速路径：索引翻目录 + 一次模型决策 ===
        // 只有“相关分类足够可信、完整候选集未超预算”才允许使用；否则继续原有的
        // L1 → L2 多轮浏览。这样优化的是默认延迟和 token，而非缩小工具发现范围。
        Optional<PlanResult> routedPlan = trySingleDecisionRoute(contextPrefix, userInput, allTools,
                failureReason, warnings, allCategories);
        if (routedPlan.isPresent()) {
            return routedPlan.get();
        }
    ...
    /**
     * 尝试走“索引召回候选 + 一次模型决策”的快速路径。
     *
     * <p>分类向量负责找到目录，Neo4j 分类快照负责展开目录下的完整工具集合，工具向量
     * 再补上跨分类或新近注册的精确候选。这里刻意不做静默截断：候选过少或超过预算
     * 时返回 empty，让调用方进入原有多轮目录浏览。这样宁可少加速，也不以漏工具为
     * 代价。</p>
     */
    private Optional<PlanResult> trySingleDecisionRoute(String contextPrefix, String userInput,
                                                          ToolCallback[] allTools, String failureReason,
                                                          List<FailureCause> warnings,
                                                          List<ToolCategoryService.CategorySummary> categories) {
        MyHelperProperties.ToolPlanner.Routing routing = runtimeTuning.effectiveRouting(props.toolPlanner().routing());
        if (routing == null || !routing.enabled()) {
            runtimeMetrics.recordRoutingFallback();
            return Optional.empty();
        }

        String retrievalKey = extractRetrievalKey(userInput);
        ToolCategoryService.CategoryCandidateRecall categoryRecall = categoryService.recallCandidateTools(
                retrievalKey, routing.categoryTopK(), routing.categoryMinScore(), categories);

        // 没有可信的分类命中时，不能假定直接工具检索覆盖了全部能力；交给旧浏览链路。
        if (!ToolRoutingPolicy.hasConfidentCategory(categoryRecall, routing)) {
            runtimeMetrics.recordRoutingFallback();
            log.info("🧭 索引路由未命中可信分类（bestScore={}）→ 保留多轮目录浏览",
                    String.format("%.2f", categoryRecall.bestScore()));
            return Optional.empty();
        }

        Set<String> availableTools = collectExisting(allTools);
        LinkedHashSet<String> candidates = new LinkedHashSet<>();

        // 工具语义检索优先补精确候选；分类展开保证 Skills/工具包不会因名称不相似而遗漏。
        for (ToolModel tool : toolRegistry.searchTools(retrievalKey, routing.directToolTopK(),
                routing.directToolMinScore())) {
            if (availableTools.contains(tool.name())) candidates.add(tool.name());
        }
        for (String toolName : categoryRecall.toolNames()) {
            if (availableTools.contains(toolName)) candidates.add(toolName);
        }

        int candidateCount = candidates.size();
        if (!ToolRoutingPolicy.hasBoundedCandidateSet(candidateCount, routing)) {
            runtimeMetrics.recordRoutingFallback();
            log.info("🧭 索引路由候选不适合一次决策（分类={}, 工具={}, 预算={}~{}）→ 多轮目录浏览",
                    categoryRecall.matchedCategories().size(), candidateCount,
                    routing.minCandidateTools(), routing.maxCandidateTools());
            return Optional.empty();
        }

        log.info("🧭 索引路由命中 {} 个分类、{} 个候选工具，跳过模型目录浏览，直接最终决策: {}",
                categoryRecall.matchedCategories().size(), candidateCount,
                categoryRecall.matchedCategories().stream().map(ToolCategoryService.CategoryMatch::name).toList());

        PlanResult result = doFinalRound(contextPrefix, allTools, userInput, failureReason, warnings,
                new ArrayList<>(candidates), List.of());
        // 快速路径里仍允许模型在候选明显不够时使用元工具扩展目录；这不是默认目录浏览，
        // 但保留了“索引漏召回也能继续发现工具”的能力边界。
        runtimeMetrics.recordRoutingFastPath();
        return Optional.of(result.withListAllToolsAllowed());
    }
    ...
    public List<String> toolNamesOf(String unitId) {
        return unitStore.getToolNamesOf(unitId);
    }
