...
public class AsyncConfig {
    ...
    /**
     * 语音片段识别必须保持顺序，同时与麦克风采集线程解耦。
     * 队列饱和时由采集线程处理，宁可短暂背压也不静默丢失用户语音。
     */
    @Bean("speechRecognitionExecutor")
    public ExecutorService speechRecognitionExecutor() {
        return new ThreadPoolExecutor(
                1, 1, 0L, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<>(8),
                r -> {
                    Thread t = new Thread(r, "speech-recognition");
                    t.setDaemon(true);
                    return t;
                },
