...
public record MyHelperProperties(
    ...
    public record Voice(
            ...
            /** 唤醒词模糊匹配允许的最大编辑距离；元优化器仅会在 0~3 内小步调整。 */
            int wakeWordMaxEditDistance,
            Beep beep
    ) {
    ...
    public record ToolPlanner(
         ...
         * 无模型目录路由配置。
         *
         * <p>快速路径只在候选分类和工具集均足够明确时启用；否则必须回到保留的
         * 多轮目录浏览流程。这样优化默认只减少模型调用，不缩小系统可发现的工具范围。</p>
         */
        public record Routing(
                boolean enabled,
                int categoryTopK,
                double categoryMinScore,
                int directToolTopK,
                double directToolMinScore,
                int minCandidateTools,
