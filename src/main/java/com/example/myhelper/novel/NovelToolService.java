...
public class NovelToolService {
    ...
    private final NovelWritingPipeline writingPipeline;
    ...
    public NovelToolService(NovelMemoryService memory,
                            ...
                            NovelWritingPipeline writingPipeline) {
        this.memory = memory;
        this.qualityChecker = qualityChecker;
        this.chapterCheckService = chapterCheckService;
        this.writingPipeline = writingPipeline;
    }
    ...
    @Tool(description = "设置本书专属写作档案。profile 应写明题材、叙事视角、时态、目标篇幅、节奏、文风、"
            + "禁忌表达、读者情绪目标和可参考的已有正文特点。它只作用于这一部小说，不会污染其他小说。")
    public String setWritingProfile(
            @ToolParam(description = "小说名称") String novelName,
            @ToolParam(description = "本书写作档案；建议包含题材/视角/节奏/篇幅/文风/禁忌/情绪目标") String profile) {
        memory.setWritingProfile(novelName, profile);
        return "✅ 《" + novelName + "》的专属写作档案已保存，后续写作和质检都会使用。";
    }

    @Tool(description = "查看本书专属写作档案。小说写得不符合预期时先检查和修改这里，而不是把某部小说的规则写进全局。")
    public String getWritingProfile(@ToolParam(description = "小说名称") String novelName) {
        return memory.getWritingProfileAsPrompt(novelName);
    }
    ...
    @Tool(description = "保存/更新某一卷的大纲。长篇（默认1000章）不要一次写完全部大纲，按卷切分、一次一卷。" +
            ...
            "secrets 本卷反泄露清单（每行一条，如 '真实身份：第45章前绝不可提'），写正文时注入防止提前剧透。")
    public String setVolume(
            ...
            @ToolParam(description = "本卷反泄露清单（可选），每行一条，如 '真实身份：第45章前绝不可提'") String secrets) {
    ...
    @Tool(description = "按质量门禁自动续写一章。后端固定执行：章节计划 → 上下文与旧章事实召回 → 草稿 → 质量评分 → "
            + "最多两轮定向修订 → 合格后一次提交正文、摘要、人物和伏笔状态。审阅失败或未达标时不会保存低质量正文。")
    public String writeNextChapterWithQuality(
            @ToolParam(description = "小说名称") String novelName,
            @ToolParam(description = "要写的章节号；不传或传 0 则写下一章") int chapterNumber) {
        if (novelName == null || novelName.isBlank()) novelName = memory.resolveNovelName(novelName);
        if (novelName == null) return "❌ 无法确定小说名称，请带上完整书名。";
        int target = chapterNumber > 0 ? chapterNumber : (int) memory.getChapterCount(novelName) + 1;
        return writingPipeline.writeAndCommit(novelName, target).asUserMessage(novelName);
    }

    @Tool(description = "手动保存用户已确认的完整章节。自动生成或自动续写必须优先调用 write_next_chapter_with_quality，"
            + "不可用本工具绕过质量门禁。content 为章节完整正文；保存后会写入图谱、落盘到文件，并触发手动章节检查。" +
    ...
    public String addChapter(
        ...
        memory.markChapterPublished(novelName, chapterNumber);
    ...
    @Tool(description = "修改已存在的章节内容。用于润色或重写。novelName 必须携带完整小说名，每次都要传；保存后会触发手动章节检查。")
    ...
    @Tool(description = "检查某一章的正文质量并给出结构化评分：重复表达、反泄露违规、伏笔冲突、剧情衔接、文风偏离、情绪目标和人物/因果逻辑。" +
            "自动续写会把它作为提交门禁；手动写作也可用它取得定向修改清单。" +
