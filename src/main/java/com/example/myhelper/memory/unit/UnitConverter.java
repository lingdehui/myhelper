 ...
 *
 * <p>复杂字段（notes/params/outputSignature/script/failureCauses/explorationRecords）
...
public class UnitConverter {
    ...
    public UnitNode toNode(Unit unit) {
        ...
        node.setScriptable(unit.scriptable());
        node.setScriptJson(toJson(unit.script()));
    ...
    public Unit toUnit(UnitNode node) {
        ...
        return new Unit(
                ...
                node.getToolName(),
                node.isScriptable(),
                parseScript(node.getScriptJson()),
                success,
                failure,
