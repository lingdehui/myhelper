 ...
 * 可靠性、验证样本量、新鲜度和经验完整度合成 0~1 质量分，供复用、脚本化升级和记忆清理共用。</p>
 */
@Service
public class ExperienceQualityService {
    ...
    /** 自动升级为直跑脚本前必须达到的质量门槛。 */
    @Value("${myhelper.experience-quality.scriptable-score:0.80}")
    private double scriptableScore;
    ...
    public double scriptableScore() {
        return scriptableScore;
    }
    ...
    private double completeness(UnitNode node) {
        ...
        if (planStep && hasMeaningfulJson(node.getScriptJson())) score += 0.35;
