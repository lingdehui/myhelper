...
public class UnitSedimentationService {
    ...
    private final UnitGraphCompactor graphCompactor = new UnitGraphCompactor();
    private final UnitDataflowInferrer dataflowInferrer = new UnitDataflowInferrer();
    ...
    public CompletableFuture<Boolean> sediment(ModelRouter.Mode mode, String userInput,
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                UnitDataflowInferrer.Inference dataflow = dataflowInferrer.infer(successful, script);
                script = dataflow.rewrittenCalls();
                ...
                for (Unit u : similar) {
                    ...
                    log.info("🔄 Unit 目标复用并增加验证次数: {}（{} 步）", shortId(u.unitId()), script.size());
                    return false;
                }

                String unitId = buildUnit(script, userInput, notes, signature,
                        dataflow.outputSignaturesByTool());
    ...
    private String buildUnit(List<ToolCallLog> script, String userInput,
        ...
        Unit unit = new Unit(
                ...
                Map.of(),
                null,
                Unit.DirectExecutionStatus.LEARNING,
                List.of(),
                List.of(),
                1,
                0,
        ...
        // 最长优先折叠已有 PLAN_STEP 公共子图；剩余步骤仍复用/注册 TOOL 叶子。
        List<UnitGraphCompactor.Invocation> invocations = graphCompactor.compact(
                script, unitStore.findReusablePlanFragments());
        int order = 1;
        for (UnitGraphCompactor.Invocation invocation : invocations) {
            String childId = invocation.isFragment()
                    ? invocation.unitId() : registerToolUnit(invocation.toolName());
            if (childId != null && !childId.equals(unitId)) {
                if (!invocation.isFragment()) {
                    unitStore.mergeOutputSignature(childId,
                            outputSignaturesByTool.getOrDefault(invocation.toolName(), Map.of()));
                }
                unitStore.linkContains(unitId, childId, order++, invocation.argumentsJson());
            }
        }
        return unitId;
    }
    ...
    private String registerToolUnit(String toolName) {
        ...
        Unit tool = new Unit(
                ...
                Unit.DirectExecutionStatus.DISABLED, List.of(), List.of(), 0, 0, 0.0,
                0.0,
