...
用户说话: "打开微信" → "给张三发消息"
            ↓              ↓
        addSegment    addSegment    (500ms 内)
            ↓              ↓
        segments: ["打开微信", "给张三发消息"]
            ↓
    500ms 无新语音 → hasPending() 返回 true
            ↓
    getFullSentence() → "打开微信给张三发消息" → 清空 segments
```

debounce 窗口由配置 `desktopbrain.dialogue.debounce-window-ms` 控制，默认 **500ms**。
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `desktopbrain.dialogue.silence-timeout-ms` | 5000 | LISTENING 状态静音超时，超时回到 IDLE |
| `desktopbrain.dialogue.debounce-window-ms` | 500 | SpeechAssembler debounce 窗口 |
