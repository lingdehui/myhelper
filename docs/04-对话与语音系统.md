...
debounce 窗口由配置 `myhelper.dialogue.debounce-window-ms` 控制，默认 **500ms**。
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `myhelper.dialogue.silence-timeout-ms` | 5000 | LISTENING 状态静音超时，超时回到 IDLE |
| `myhelper.dialogue.debounce-window-ms` | 500 | SpeechAssembler debounce 窗口 |
