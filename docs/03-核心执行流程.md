...
当 Layer 1、Layer 2 均未命中时调用，按 docs/临时文件 设计的多轮渐进式交互：
...
**防死循环与异常**：路径记忆（visitedIds，重选拒绝）、禁止回退、maxCategoryRounds 限制、工具列表 > 50 自动截断、分类名不存在→模糊匹配候选→AI 重试、AI 不选分类→searchTool 路径传前 50 个工具、非 JSON 响应优雅降级
...
系统在空闲时基于能力清单自动决策和执行探索学习任务。学习目标送入 **主执行管线**（`TurnProcessor`），走完整的"计划 → 执行 → 反思 → 入库"流程。学习成果以 **ACTIVE Episode** 形式持久化到 Qdrant `episodes` 集合。
...
`searchTool(keyword, keywordEn)` — 两个入参：
- 中文搜不到 → 自动用英文关键词兜底匹配工具名+描述
- 探索学习输入硬注入提示：`"务必先用 listAllTools，searchTool 必须传中英双语参数"`
