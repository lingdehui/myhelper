...
脚本化条件：无递归子步骤 + 无数据流依赖 + 步骤数 ≤ 5（或 `successCount ≥ 5 && stability > 0.9 && qualityScore ≥ 0.80`）+ 无 FALLBACK/DISABLES。
...
- `stability > 0.9 && successCount ≥ 5 && qualityScore ≥ 0.80`：可脚本化跳过 AI
...
| 拼接 | `String text` | `speechAssembler.addSegment()` / `getFullSentence()` | `String complete` | `debounceWindowMs = 300` |
