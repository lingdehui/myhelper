...
AI 执行失败
  ...
  → FailureExperienceHandler.handle()  // 滑动窗口聚合
    记录到滑动窗口（1h 窗口，3 次阈值）
    达阈值 → 生成 FailurePattern → 持久化到 Qdrant + Neo4j
  → 分支处理：
...
**来源**：`RuleInductionService` 每天凌晨 3 点扫描 FailurePattern + 成功 Episode，调用 LLM 跨案例归纳生成通用规则，存入 Neo4j `(:Rule)` 节点。
...
`DesktopBrainApplication.processAITurn()` 原有的 ~400 行 AI 执行逻辑已抽离为独立的 `TurnProcessor` Spring @Component：

```
DesktopBrainApplication (~790行)
  ├── 语音采集、VAD、ASR、拼接（保留）
...
TurnProcessor (~420行)
  ...
  ├── handleCacheHit()     → 缓存命中 + AI判可用 + 脚本化/非脚本化
  ├── handleNewPlan()      → DRAFT → 执行 → Reflect → ACTIVE/FAILED
  ├── handleCacheFailure() → 重新规划 + 重试
  ├── executeWithTools()   → 过滤工具 + LoggingToolCallback 收集日志
...
系统在空闲时基于失败经验和知识库状态，自动决策和执行探索学习任务，将学习成果沉淀为 `KnowledgeSnippet`，实现"越用越聪明"。
...
| 探索决策 | `AutonomousExplorationService` | 收集上下文 → 本地 LLM 决策 → 提交执行 |
| 探索执行 | `ExplorationExecutor` | 逐步执行 + 自我评估 + 知识沉淀 |
...
IdleDetectionService.shouldExplore()
  │
  ├── 1. 功能开关检查：desktopbrain.exploration.enabled
  │
  ├── 2. 空闲时间检查：
  │     当前时间 - DialogStateMachine.lastInteractionTime ≥ idle-threshold (默认30min)
  │
  ├── 3. 时段检查：
  │     blackout-hours 为空 → 全天可行
  │     blackout-hours 非空 → 当前小时必须在列表中
  │
  └── 4. 频率限制（只检查不消费）：
        当前小时已探索次数 < max-sessions-per-hour (默认3)
```

**触发方式**：
- **自动**：`@Scheduled(fixedDelay=5min)` → 每 5 分钟巡检
- **手动**：AI 调用 `ExplorationTool.startAutonomousLearning(topic)`

### 5.3 探索执行流程

```
AutonomousExplorationService.tryExplore()
  │
  ├── idleDetection.shouldExplore() ← 仅检查，不消费配额
  │
  ├── 异步执行：
  │   ├── idleDetection.consumeQuota() ← 确认执行后消费配额
  │   ├── buildContext() → 收集失败模式 + 知识库状态
  │   ├── decide(context) → 本地 qwen2.5:3b 决策
  │   │     ├── SKIP → 退出
  │   │     └── LEARN → 生成 ExplorationDecision
  │   │
  │   └── executor.execute(decision) → 异步执行
  │         │
  │         ├── 1. createDraft() → Qdrant DRAFT Episode
  │         ├── 2. executeSteps() → 逐步执行
  │         │     ├── executeStep() → 本地 LLM + MCP工具调用
  │         │     ├── 成功步 → recordSubEpisode (ATOMIC)
  │         │     └── 失败步 → FailureExperienceHandler.handle()
  │         ├── 3. selfAssess() → AI 自我评估
  │         ├── 4. activateDraft() / failDraft() → 更新 Episode
  │         └── 5. saveKnowledgeSnippet() → 生成知识片段
  │
  └── 不阻塞调用方
```
...
desktopbrain:
  exploration:
    enabled: true
    idle-threshold-minutes: 30       # 空闲多久触发
    max-sessions-per-hour: 3         # 每小时上限
    max-duration-minutes: 15         # 单次最长
    blackout-hours: []               # 留空=全天
    model: qwen2.5:3b                # 本地模型
  memory-maintenance:
    enabled: true
    cron: 0 0 4 * * ?               # 凌晨4点
    threshold: 0.7                   # 触发清理占用率
    target: 0.5                      # 清理目标占用率
```
...
| 决策 | 内容 |
|------|------|
| LLM 路由 | 主对话 → DeepSeek；探索全部 → 本地 Ollama qwen2.5:3b |
| 配置位置 | 探索引擎独立 YAML (`desktopbrain-exploration.yml`)，通过 `spring.config.import` 引入 |
| 异步模型 | 决策和执行均通过 `CompletableFuture.runAsync()`，不阻塞 |
| 容错 | JSON 解析失败 → 返回 SKIP/null，不崩溃 |
